    ...
    function initializeDOM() {
        ...
        elements.cursor.style.width = `${CONFIG.cursor.radius * CONFIG.cursor.scale.normal}px`;
        elements.cursor.style.height = `${CONFIG.cursor.radius * CONFIG.cursor.scale.normal}px`;
        ...
        elements.cursor.style.transition = "transform 0.2s ease, width 0.2s ease, height 0.2s ease";
        ...
        const controls = [
            ...
            { id: "warpStrength", label: "Gravity Force", value: CONFIG.grid.warpStrength },
            { id: "maxDistance", label: "Max Distance", value: CONFIG.grid.maxDistance },
            { id: "maxDisplacement", label: "Max Displacement", value: CONFIG.grid.maxDisplacement }
        ];

        elements.controls.innerHTML = controls.map(control => `
            <label>
                ${control.label}: 
                <input type="range" id="${control.id}Range" 
                    ...
                    value="${control.value}" 
                    oninput="syncControl('${control.id}', this.value)">
                <input type="number" id="${control.id}Input" 
                    min="${CONFIG.controls.ranges[control.id].min}" 
                    max="${CONFIG.controls.ranges[control.id].max}" 
                    value="${control.value}" 
                    onchange="syncControl('${control.id}', this.value)">
            </label>
        `).join('');
    ...
    function initializeCustomCursor() {
        try {
            ...
            Object.assign(elements.cursorOuter.style, {
                ...
                zIndex: "9999",
                transform: "translate(-50%, -50%)",
                transition: "transform 0.2s ease, width 0.2s ease, height 0.2s ease, box-shadow 0.2s ease, backdrop-filter 0.2s ease",
            ...
            state.cursor.current = { x: window.innerWidth / 2, y: window.innerHeight / 2 };
    ...
    function updateCursor(e) {
        if (!elements.cursorInner || !elements.cursorOuter) return;
    ...
    function addEventListeners() {
        ...
        document.querySelectorAll("button, a, input").forEach((element) => {
            element.addEventListener("mouseenter", () => setCursorSize(CONFIG.cursor.outer.scale.hover));
            element.addEventListener("mouseleave", () => setCursorSize(CONFIG.cursor.outer.scale.normal));
        });
    ...
    function syncControl(controlId, value) {
        const numValue = parseInt(value);
        CONFIG.grid[controlId] = numValue;
        
        document.getElementById(`${controlId}Range`).value = numValue;
        document.getElementById(`${controlId}Input`).value = numValue;
